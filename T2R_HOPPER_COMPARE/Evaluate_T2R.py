'''from stable_baselines3 import PPO, PPO, TD3, A2C
...
p = "T2R_HOPPER_COMPARE\GTP_PPO\Hopper-v4\models\latest_model_cont.zip"
learner = PPO.load(p)

venv = make_vec_env(
    "Hopper-v4",
    rng=np.random.default_rng(SEED),
    n_envs=1, # número de entornos paralelizados
    post_wrappers=[
        lambda e, _: RolloutInfoWrapper(e)
    ],
)
...
import seals                              # registra 'seals/Hopper-v1' en Gym
import numpy as np
from imitation.data.wrappers import RolloutInfoWrapper
...
MODEL_PATH   = p     # debe corresponder a "PPO_seals_model.zip"
VIDEO_LENGTH = 1000
os.makedirs(VIDEO_FOLDER, exist_ok=True)

# 1) Factory para crear el entorno con render_mode y wrapper de rollout
def make_seals_env():
...
vec_env = DummyVecEnv([make_seals_env])

# 3) Envuélvelo con VecVideoRecorder (solo graba al inicio, VIDEO_LENGTH pasos)
...
# 4) Carga tu agente PPO (asegúrate de que MODEL_PATH.zip exista)
model = PPO.load(MODEL_PATH, env=vec_env)


# 5) Ejecuta y graba (reset devuelve solo obs)
for i in range(10):
    obs = vec_env.reset()
